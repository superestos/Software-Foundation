...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros X l m n. induction l as [| x l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite -> IHl'. reflexivity.
Qed.
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  ...
  | nil => ([], [])
  | (x, y) :: t =>
    match split t with (xlist, ylist) => (x :: xlist, y :: ylist) end
  end.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => if (7 <? n) then even n else false) l.
...
Proof.
  intros X Y f l1 l2. induction l1 as [| n l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite IHl'. reflexivity.
Qed.
...
Proof.
  simpl. destruct p as [x y]. reflexivity.
Qed.
(** [] *)
...
(* n X f x means we apply f to x n times, this result would be given to f.*)
